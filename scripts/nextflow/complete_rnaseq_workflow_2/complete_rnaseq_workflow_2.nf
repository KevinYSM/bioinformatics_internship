...
process GATK_mark_duplicates{
        ...
        output:
        path '*.MarkDuplicates.bam'

        """
        GATK_mark_duplicates.sh $INDEXED_bam_file 
        """
}
process SAM_re_index{
        publishDir params.outdir, mode:'copy'
        input:
        file MARK_duplicates_file

        output:
        path '*.txt'

        """
        samtools index -@ 5 $MARK_duplicates_file
        """
}
...
workflow{
        ...
        GATK_duplicates_ch=SAM_sort(aligned_bam_files)
        //GATK_duplicates_ch=SAM_index(SAM_index_ch)
        SAM_re_index_ch=GATK_mark_duplicates(GATK_duplicates_ch)
        GATK_split_ch=SAM_re_index(SAM_re_index)
        GATK_recalibrator_ch=GATK_split_ch(GATK_split_ch)
        GATK_base_recalibration(GATK_recalibrator_ch)
